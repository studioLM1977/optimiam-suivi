...
function renderAjouter() {
  ...
  root.innerHTML = `
    ...
    <div class="toggle-pill" id="basefood-cat-toggle">
      ${["Féculents", "Fruits", "Légumes"].map((cat, i) => `<button type="button" data-cat="${cat}" class="${i === 0 ? "active" : ""}">${cat}</button>`).join("")}
    </div>
    <div class="search-results" id="basefood-results"></div>
  ...
  renderBaseFoodResults("Féculents");
  document.getElementById("basefood-cat-toggle").querySelectorAll("[data-cat]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      renderBaseFoodResults(btn.dataset.cat);
    });
  });
...
function renderBaseFoodResults(cat) {
  const container = document.getElementById("basefood-results");
  const items = BASE_FOODS.filter((f) => f.cat === cat);
